...
document.getElementById('modal-overlay').addEventListener('click', e => { if (e.target === document.getElementById('modal-overlay')) closeCatalogModal(TOOLS_IDS); });
document.addEventListener('keydown', e => { if (e.key === 'Escape') closeCatalogModal(TOOLS_IDS); });

const _toolsBtnFullscreen = document.getElementById('modal-fullscreen-btn');
if (_toolsBtnFullscreen) _toolsBtnFullscreen.addEventListener('click', () => toggleCatalogFullscreen('modal-canvas-wrap'));
