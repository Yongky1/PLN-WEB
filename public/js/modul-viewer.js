...
function closeMeshPanel() {
  ...
  if (specTitle) {
    specTitle.textContent = 'Komponen Konstruksi';
  }
  
  // Restore all materials
  renderBomList(moduleMaterials, moduleTools);
}
...
function buildMatCard(row) {
  ...
  const thumb = m.image
    ...
    : `<span>📦</span>`;
  const qtyBadge = qty > 1 ? `<span class="card-qty">× ${qty}</span>` : '';
...
function renderBomList(mats, tools) {
...
function showMeshPanel(mesh) {
  ...
  if (finalDisplayName === meshName) {
    if (hasAnyMaterialMapping && mats.length > 0 && mats[0].material) {
...
function init() {
  ...
  if (assetsEl) {
    try {
      currentAssets = JSON.parse(assetsEl.textContent) || [];
    } catch (_) {
      currentAssets = [];
    }
  }
