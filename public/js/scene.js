...
const materials = {
  ...
  // Other buildings
  institution: new THREE.MeshPhysicalMaterial({
    color: 0xdddddd,
    roughness: 0.2,
    metalness: 0.5,
    clearcoat: 0.5,
...
function createLandmarkBuilding(x, z) {
  ...
  const centerBlock = new THREE.Mesh(
    new THREE.BoxGeometry(centerW, centerH, centerD),
    materials.plnWall
  );
...
function createInstitutionBuilding(x, z, h) {
  ...
  const p2 = new THREE.Mesh(
    new THREE.BoxGeometry(w * 0.8, h * 0.4, d * 0.8),
...
function createLampPost(x, z) {
  ...
  const pole = new THREE.Mesh(
    new THREE.CylinderGeometry(0.2, 0.2, h),
...
function updateLightingAndMaterials() {
  ...
  const currentColor = new THREE.Color()
    .copy(colors.daySky)
    .lerp(colors.nightSky, transitionProgress);
  ...
  windowMaterials.forEach((m) => {
    m.emissiveIntensity = transitionProgress * 3.0;
  });
  houseLightMaterials.forEach((m) => {
    m.emissiveIntensity = transitionProgress * 4.0;
  });
  streetLightMaterials.forEach((m) => {
    m.emissiveIntensity = transitionProgress * 6.0;
  });

  trafficLights.forEach((t) => {
    t.mat.opacity = 0.1 + transitionProgress * 0.9;
  });
