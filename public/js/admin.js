const PAGE_META = {
  ...
  'add-konstruksi': {
    title: 'Tambah Konstruksi',
    subtitle: 'Tambah dan kelola data konstruksi jaringan',
  },
  'add-material': {
    title: 'Tambah Material',
    subtitle: 'Tambah dan kelola katalog material jaringan',
  },
  'add-tools': {
    title: 'Tambah Tools & Alat K3',
    subtitle: 'Tambah dan kelola katalog alat lapangan',
  },
...
function showToast(msg) {
  ...
  t.textContent = msg;
  t.style.cssText =
    'position:fixed;bottom:28px;right:28px;z-index:9999;background:#00E5FF;color:#05101E;padding:12px 20px;border-radius:12px;font-size:13px;font-weight:700;font-family:inherit;box-shadow:0 8px 24px rgba(0,229,255,0.3);animation:fadeIn 0.2s ease;';
...
function addKonstruksi() {
  ...
  if (!name) {
    alert('Nama konstruksi tidak boleh kosong.');
    return;
  }

  const item = document.createElement('div');
  item.style.cssText =
    'background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:16px 20px;display:flex;align-items:center;gap:16px;';
  item.innerHTML = `
...
function addMaterial() {
  ...
  if (!name || !code) {
    alert('Nama dan kode material tidak boleh kosong.');
    return;
  }
...
function resetForm(type) {
  if (type === 'konstruksi') {
    ...
    ['tools-name', 'tools-icon', 'tools-standard', 'tools-desc'].forEach(
